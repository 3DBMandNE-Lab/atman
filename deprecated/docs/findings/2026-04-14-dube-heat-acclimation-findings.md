...
1. **Ingest** raw Olink Explore NGS CSVs (122,135 NPX rows, 43 samples) via `atman ingest`.
2. **QC** applies the Dube rule (`NPX → missing` iff `QC_Warning != PASS` OR `Assay_Warning != PASS`) — masks 3,725 / 122,135 rows (3.0%).
3. **Differential abundance** via `atman de`:
...
```bash
cargo test -p atman --test dube_de_sanity -- --nocapture
```
