...
For each pathway P in {hallmark, kegg, go_bp}:
    P_score(subject) = mean of gene-level dNPX_<contrast>(gene, subject)
                       over all genes in P that are in the Olink universe
    (if a gene appears in multiple assays/panels, assay values are averaged
     per gene first, then genes are averaged for pathway score)
...
def pathway_scores(
    ...
    # Build gene -> {subject: [values]} from dnpx.
    # Some genes can appear in multiple assays/panels; aggregate at gene level
    # first so pathway scores are not driven by arbitrary assay overwrite order.
    gene_vals: Dict[str, Dict[str, List[float]]] = defaultdict(lambda: defaultdict(list))
    for (_oid, gene, _panel), per_subject in dnpx.items():
        if gene in pathway_genes:
            for s, v in per_subject.items():
                if math.isfinite(v):
                    gene_vals[gene][s].append(v)

    # Collapse replicate assay entries to one gene-level value per subject.
    gene_subject_mean: Dict[str, Dict[str, float]] = {}
    for gene, subj_vals in gene_vals.items():
        gene_subject_mean[gene] = {
            s: float(np.mean(vals)) for s, vals in subj_vals.items() if vals
        }

    scores: Dict[str, float] = {}
    for s in PARTICIPANTS:
        contributions = [
            v[s] for v in gene_subject_mean.values() if s in v and math.isfinite(v[s])
        ]
