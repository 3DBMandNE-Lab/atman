...
| `k_selection_applied` | `decompose ica` | Whether `--k-selection` chose k, or `--k` overrode it. `args` keeps what was requested. This records which of the two decided, so a replay does not have to infer it from the presence of a flag. |
