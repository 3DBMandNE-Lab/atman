#!/usr/bin/env python3
"""Universe-corrected pathway enrichment for karnaProteome de_results.tsv.
...
def bh_fdr(p_values: list[float]) -> list[float]:
    ...
    q = [0.0] * m
    for rank, (orig, p) in enumerate(indexed, start=1):
        q[orig] = min(1.0, p * m / rank)
    # Monotonicity: sweep ascending p, ensure q_i <= q_{i+1}.
    last = 1.0
    for _, orig in [(None, o) for o, _ in reversed(indexed)]:
        pass
    # Easier: compute q_sorted with reverse sweep, then scatter back.
...
def main() -> int:
    ...
    ap.add_argument("de_results", help="karnaproteome de_results.tsv")
